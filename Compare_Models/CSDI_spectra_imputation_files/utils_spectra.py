...
#     )
    ...
    lr_scheduler1 = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100], gamma=0.1)
    ...
    def custom_lr_scheduler(epoch):
        if epoch <= 100:
            ...
            return 0.5 * (1 + math.cos(math.pi * (epoch - 100) / (config["train"]["epochs"] - 100)))

    lr_scheduler2 = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=custom_lr_scheduler)
            ...
            with torch.no_grad():
                with tqdm(valid_loader, mininterval=5.0, maxinterval=50.0) as it:
                    for batch_no, valid_batch in enumerate(it, start=1):
                        loss = model(valid_batch, is_train=1) #change to 0 if it doesnt work
